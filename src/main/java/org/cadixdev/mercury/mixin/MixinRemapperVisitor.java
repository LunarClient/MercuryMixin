...
public class MixinRemapperVisitor extends ASTVisitor {
    ...
    public boolean visit(final MethodDeclaration node) {
        ...
        final ClassMapping<?, ?> target = this.mappings.getOrCreateClassMapping(mixinTargetName);
            ...
            if (Objects.equals(PROXY_CLASS, annotationType)) {
                for (final IMemberValuePairBinding pair : annotation.getDeclaredMemberValuePairs()) {
                    if (Objects.equals("value", pair.getName())) {
                        ...
                        Expression value = originalAnnotation.getValue();
                        if (value instanceof StringLiteral || value instanceof InfixExpression) {
                            replaceExpression(ast, this.context, value, deobf);
                        }
                    }
                }
            }
